...
app.innerHTML = `
  ...
  <div id="hint">WASD to move · Walk onto construction pads to build</div>
`;
...
function createPad(x, z, label, cost, action) {
  ...
  pad.userData = { label, cost, action, built: false };
  pads.push(pad);
  return pad;
}

createPad(-8, -5, 'Market Stall', 100, () => {
  box(-8, 1.5, -5, 4, 3, 3, 0xe58b3d);
  box(-8, 3.5, -5, 4.5, 0.4, 3.5, 0x8e3e2f);
  population += 2;
  populationEl.textContent = population;
});

createPad(8, -5, 'Bakery', 250, () => {
  box(8, 2, -5, 5, 4, 4, 0xf0d6a5);
  box(8, 4.5, -5, 5.5, 0.5, 4.5, 0xb65c42);
  population += 4;
  populationEl.textContent = population;
});
...
function animate(now) {
  ...
  for (const pad of pads) {
    if (!pad.userData.built && player.position.distanceTo(pad.position) < 2.2) {
